...
class RaftNode:
    ...
    def __print_log(self, text: str, end='\n'):
        typeNode = "Follower" if self.type == RaftNode.NodeType.FOLLOWER else "Leader" if self.type == RaftNode.NodeType.LEADER else "Candidate"
        print(f"[{self.address}] [{time.strftime('%H:%M:%S')}] [{typeNode}] {text}", end=end)
    ...
    async def __leader_heartbeat(self, func="heartbeat", args=None):
        while (self.type == RaftNode.NodeType.LEADER):
            self.__print_log(f"Sending {func}...")
            self.__print_log(f"Cluster Addr List: {self.cluster_addr_list}")
            ack_count = 1
            responses = []
            ...
            for node_addr in self.cluster_addr_list:
                if node_addr != self.address:
                    self.__print_log(f"Sending {func} to {node_addr}")
            ...
            if ack_count > len(self.cluster_addr_list) // 2:
                ...
                self.__print_log(f"Entries not committed. Acknowledged by {ack_count} out of {len(self.cluster_addr_list)} followers.")
